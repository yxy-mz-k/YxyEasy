...
export function usePermission() {
  ...
  function hasPermissions(
    ...
    if (PermissionModeEnum.BACK === permMode) {
      const allCodeList: any = permissionStore.getRoleBtnList;
      if (!isArray(value)) {
        return allCodeList.has(value);
      }
      let btnMap: any = [];
      allCodeList.forEach((key, value) => {
        btnMap.push(value);
      });
      return (intersection(value, btnMap) as string[]).length > 0;
    }
    return true;
  }
