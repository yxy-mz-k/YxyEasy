...
export default defineComponent({
  ...
  setup(props, { attrs, slots, emit, expose }) {
    ...
    function handleSearch(searchValue: string, from = "input") {
    ...
    const treeData = computed(() => {
      ...
      eachTree(data, (item, _parent) => {
        ...
        const iconDom = icon ? (
          ...
          <span class="ml-2">{getSlot(slots, "icon")}</span>
        ) : null;
    ...
    return () => {
      ...
      const showTitle =
        title || toolbar || search || slots.headerTitle || slots.treeSearch;
      const scrollStyle: CSSProperties = { height: "calc(100% - 38px)" };
      return (
        <div class={[bem(), "h-full", attrs.class]}>
          {showTitle &&
            (getSlot(slots, "treeSearch") ?? (
              <TreeHeader
                checkable={checkable}
                checkAll={checkAll}
                expandAll={expandAll}
