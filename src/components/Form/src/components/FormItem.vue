...
export default defineComponent({
  ...
  setup(props, { slots }) {
    ...
    function renderLabelHelpMessage() {
      const { label, helpMessage, helpComponentProps, subLabel, field } =
        props.schema;
      ...
      const getHelpMessage = isFunction(helpMessage)
        ...
        : helpMessage;
      // 1. 优先检查特定字段插槽 #label_{field}
      const labelSlotName = `label_${field}`;
      let labelContent: any = null;
      if (slots[labelSlotName]) {
        // 使用特定字段插槽
        labelContent = getSlot(slots, labelSlotName, {
          schema: props.schema,
        ...
        // 使用全局 #label 插槽
        labelContent = getSlot(slots, "label", {
          schema: props.schema,
          model: props.formModel,
          field,
        });
