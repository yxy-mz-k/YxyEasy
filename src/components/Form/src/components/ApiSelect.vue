...
const props = defineProps({
  ...
  api: {
    type: Function as PropType<
      (arg?: any) => Promise<OptionsItem[] | Recordable>
    >,
    default: null,
  },
  // api params
  ...
  try {
    ...
    let apiParams =
      Object.getPrototypeOf(params) === Object.prototype
        ? assignIn({}, params, searchParams.value)
        : params;
    ...
    if (resultField) {
      ...
      optionsRef.value = get(res, "records") || [];
    }
    emitChange();
  } catch (error) {
    console.warn(error);
  ...
  if (visible) {
    if (props.alwaysLoad) {
      ...
      if (
        !(
          !!props.apiSearch &&
          !!props.apiSearch.show &&
          !props.apiSearch.emptySearch
        )
      ) {
        await fetch();
      } else {
