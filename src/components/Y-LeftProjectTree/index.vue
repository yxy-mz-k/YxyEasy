...
//     const findIndex = self.findIndex((s: any) => s?.[attrs?.fieldNames?.key || 'id'] == key);
//     if (findIndex > -1) {
...
const handleInitFirstSelect = () => {
  ...
  if (selectedKeys.value?.length) {
    const selectedNodes = selectedKeys.value
      ?.map((s: any) => (tKeyMap.has(s) ? tKeyMap.get(s) : null))
      ?.filter(Boolean);
    const names = selectedKeys.value
      ?.map((s: any) =>
        tKeyMap.has(s) ? tKeyMap.get(s)?.[attrs?.fieldNames?.title] : null,
      )
      ?.filter(Boolean);
    emits("select", selectedKeys.value, { selectedNodes }, names);
    return;
  }
  if (attrs?.selectedKeys?.length) {
    selectedKeys.value = attrs?.selectedKeys;

    const selectedNodes = attrs?.selectedKeys
      ?.map((s: any) => (tKeyMap.has(s) ? tKeyMap.get(s) : null))
      ?.filter(Boolean);
    const names = attrs?.selectedKeys
      ?.map((s: any) =>
        tKeyMap.has(s) ? tKeyMap.get(s)?.[attrs?.fieldNames?.title] : null,
      )
      ?.filter(Boolean);
    emits("select", attrs?.selectedKeys, { selectedNodes }, names);
    return;
  }
  if (attrs?.selectFirst) {
    let newdata = attrs?.treeData?.filter((item: any) => {
      return (
...
const handleTree = (data: any) => {
  data?.map((d: any) => {
    tKeyMap.set(d?.[attrs?.fieldNames?.key], d);
    if (d?.[attrs?.fieldNames?.children]?.length) {
      handleTree(d?.[attrs?.fieldNames?.children]);
    }
  });
};
watch(
