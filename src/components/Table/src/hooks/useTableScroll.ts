...
export function useTableScroll(
  ...
  function caclFooterHeight(tableEl: Element): number {
    ...
    if (!isBoolean(pagination)) {
      ...
      const offsetHeight = footerEl?.offsetHeight ?? 0;
      footerHeight += offsetHeight || 0;
    }
    return footerHeight;
  }

  function calcHeaderHeight(tableEl: Element): number {
    let headerHeight = 0;
    if (!headEl) {
      headEl = tableEl?.querySelector(".ant-table-thead") as HTMLElement;
    }
    const offsetHeight = headEl?.offsetHeight ?? 0;
    headerHeight += offsetHeight || 0;

    return headerHeight;
  }

  function calcTitleHeight(tableEl: Element): number {
    let titleHeight = 0;
    if (!titleEl) {
      titleEl = tableEl?.querySelector(".ant-table-title") as HTMLElement;
    }
    const offsetHeight = titleEl?.offsetHeight ?? 0;
    titleHeight += offsetHeight || 0;
  ...
  function getSearchFormHeight() {
    const { useSearchForm } = unref(propsRef);
    const table = unref(tableElRef);
    if (!table) return 0;
    const tableEl: Element = table.$el;
    if (unref(wrapRef) && useSearchForm) {
      return (
        tableEl.parentElement?.querySelector(".ant-form")?.offsetHeight + 16
      );
    }
    return 0;
  }
  ...
  async function calcTableHeight() {
    ...
    const headerHeight = calcHeaderHeight(tableEl);
    const titleHeight = calcTitleHeight(tableEl);
    // let bottomIncludeBody = calcBottomAndPaddingHeight(tableEl, headEl);
    // if (!isCanResizeParent) {
    //   bottomIncludeBody = bottomIncludeBody - 22;
    // }
    const modalElIterator: HTMLElement = tableEl.parentElement!.parentElement!;
    const ofh = modalElIterator.offsetHeight;

    const modalHeight = calcModalHeight(tableEl);

    const marginPaddingHeight = getMarginPaddingHeight();

    const formHeight = getSearchFormHeight();
    ...
    let height = Math.floor(
      ofh -
        ...
        // modalHeight -
        // 页面 footer 高度（非弹窗的时候）
        // (getShowFooter.value && modalHeight <= 0
        //   ? isCanResizeParent
        //     ? 0
        //     : layoutFooterHeight
        //   : 0) -
