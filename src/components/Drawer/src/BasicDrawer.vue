<template>
  <Drawer
    ...
    :class="[
      prefixCls,
      {
        scrollWrapperNoPadding: getBindValues?.scrollWrapperNoPadding,
      },
    ]"
    @close="onClose"
  >
...
watch(
  ...
  (visible) => {
    nextTick(() => {
      ...
      if (instance && drawerInstance.emitVisible) {
        drawerInstance.emitVisible?.(visible, instance.uid);
      }
    });
  },
);
