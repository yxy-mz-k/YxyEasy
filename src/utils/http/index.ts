...
import { setObjToUrlParams, deepMerge } from "utils/other";
...
const transform: AxiosTransform = {
  ...
  transformRequestHook: (
    ...
    const hasSuccess =
      ...
      (code === ResultEnum.SUCCESS || ResultEnum.SUCCESS2 || code === 412);
    if (hasSuccess && success.toString() == ResultEnum.RESULTSUCCESS) {
      return result;
    }
  ...
  beforeRequestHook: (config, options) => {
    ...
    if (joinPrefix) {
      config.url = urlPrefix ? `${urlPrefix}${config.url}` : `${config.url}`;
    }
  ...
  requestInterceptors: (config, options) => {
    ...
    // return config;
    // 请求之前处理config
    // (config as Recordable).headers.Authorization = 'Basic d2ViQXBwOndlYkFwcA==';
  ...
  responseInterceptorsCatch: (axiosInstance: AxiosResponse, error: any) => {
    ...
    checkStatus(error?.response?.status, msg, result, errorMessageMode);
