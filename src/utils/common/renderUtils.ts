...
import { getOptionsByDictCode, getDictName } from "utils/dicUtil.js";
...
const render = {
  ...
  renderDict: (v, code, renderTag = false) => {
    const text = "";
    const obj = getOptionsByDictCode(code);
   ...
   * @param renderTag 是否使用tag渲染
   */
  renderDictName: (v, code, renderTag = false) => {
    const text = getDictName(code, v);
    return isEmpty(text) || !renderTag ? h("span", text) : h(Tag, text);
  },
